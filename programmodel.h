...
public:
    ...
    QList<BaseTOP*> get_entry_nodes();

    QList<BaseTOP*> get_all_nodes();
...
public slots:
    ...
    void remove_operator_undoable(QPointer<BaseTOP> operator_);

    void move_operator_undoable(qint64 id, int x, int y);

    void connect_operators_undoable(QPointer<BaseTOP> operator_a, QPointer<BaseTOP> operator_b, int b_input);

    void disconnect_operators_undoable(QPointer<BaseTOP> operator_a, QPointer<BaseTOP> operator_b, int b_input);
...
signals:
    ...
    void operator_added(BaseTOP* pointer, qint64 id);
